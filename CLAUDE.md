# Orion Theme - Shopline OS 3.0 Development Guide

## Project Overview

Orion is a modern Shopline OS 3.0 theme successfully converted from OS 2.0, built with:

- **Tailwind CSS 3** for styling
- **Vue 3** for reactivity and component logic
- **Vite** for building and bundling
- **Shopline OS 3.0** theme architecture with proper Sline templating syntax
- **Sline Templating** (not Handlebars) - Shopline's OS 3.0 template engine
- **Components Architecture** using `{{#component}}` syntax
...
```
theme/
├── blocks/              # External blocks for sections
├── components/          # Reusable components (Sline syntax)
├── i18n/                # Translation files (OS 3.0)
│   ├── en.json          # General theme translations
│   └── en.schema.json   # Schema/settings translations
├── layout/              # Main theme layouts
│   └── theme.html       # Main layout file
├── public/              # Static assets and compiled files
│   ├── base/            # Base JavaScript and CSS
│   └── assets/          # Images, fonts, etc.
├── sections/            # Theme sections (folder structure)
│   ├── header/          # Header section
│   ├── footer/          # Footer section
│   ├── hero-banner/     # Hero banner section
│   └── ...              # Other sections
├── templates/           # Page templates (JSON)
├── theme.config.json    # Theme configuration (OS 3.0)
└── theme.schema.json    # Theme settings schema (OS 3.0)
```
...
- **Config** - theme.config.json, theme.schema.json (OS 3.0 format)
- **i18n** - en.json, en.schema.json with all translations (OS 3.0 format)
- **Components** - Reusable Sline components (meta-tags, icons, pagination, etc.)
...
4. **Sline Template Helpers**

   Common helpers used in Shopline OS 3.0 templates:
   ...
   Note: Sline uses similar syntax to Handlebars but with OS 3.0 specific features
...
1. **`i18n/en.json`** - General theme translations
...
2. **`i18n/en.schema.json`** - Schema translations (IMPORTANT)
...
- The `image_url` filter only works in Sline templates, not in JavaScript
...
Shopline = {
  ...
  // Theme settings from theme.schema.json
  theme: {
    settings: {
      cart_type: "drawer",
...
- Global settings from `theme.schema.json` may not be directly accessible in section templates using `{{settings.variable}}` - use JavaScript to access them via `Shopline.theme.settings`
...
4. **Text Content**
   - Use translation files (`i18n/en.json`, `i18n/en.schema.json`)
...
- Follow Shopline 3.0 best practices
