...
// Don't use custom element for ReviewSummary to avoid style issues
...
function mountVueComponents(container = document) {
  ...
  productCardMounts.forEach((mount) => {
    ...
    // Check if product JSON is passed
    const productAttr = mount.getAttribute(":product");
    if (productAttr) {
      try {
        props.product = JSON.parse(productAttr);
      } catch (e) {
        console.error("Failed to parse product JSON:", e);
      }
    }
    
    // Fallback to individual attributes if no product object
    if (!props.product) {
      props = {
  ...
  // Mount ReviewSummary components (as Vue apps to avoid style isolation)
  const reviewSummaryMounts = container.querySelectorAll("reviews-summary");
  reviewSummaryMounts.forEach((mount) => {
    if (mount._vueApp) return;
    ...
    const app = createApp(ReviewSummary, props);
    mount._vueApp = app;
    app.mount(mount);
  });


  // Initialize QuickView modal (only once, at document level)
  if (container === document && !document.querySelector('#quick-view-mount')) {
    const quickViewMount = document.createElement('div');
    quickViewMount.id = 'quick-view-mount';
    document.body.appendChild(quickViewMount);
    
    const quickViewApp = createApp(QuickView);
    const quickViewInstance = quickViewApp.mount(quickViewMount);
    window.QuickViewModal.setInstance(quickViewInstance);
  }
...
// Create global QuickView instance
if (!window.QuickViewModal) {
  let quickViewInstance = null;
  window.QuickViewModal = {
    open(productData) {
      if (quickViewInstance && quickViewInstance.openModal) {
        quickViewInstance.openModal(productData);
      }
    },
    
    openWithHandle(productHandle) {
      if (quickViewInstance && quickViewInstance.openModalWithHandle) {
        quickViewInstance.openModalWithHandle(productHandle);
      }
    },
    
    close() {
      if (quickViewInstance && quickViewInstance.closeModal) {
        quickViewInstance.closeModal();
      }
    },
    
    setInstance(instance) {
      quickViewInstance = instance;
    }
  };
}
...
if (!window.OrionCart) {
  ...
  // Create OrionWishlist system for managing wishlisted products
  window.OrionWishlist = {
    items: [], // Array of product IDs only
    productCache: {}, // Cache for fetched product data
    currentNotification: null, // Track current notification
    notificationTimeout: null, // Track notification timeout
    
    // Initialize wishlist from localStorage
    init() {
      try {
        const stored = localStorage.getItem('orion_wishlist');
        if (stored) {
          const data = JSON.parse(stored);
          // If old format (array of objects), convert to new format (array of IDs)
          if (data.length > 0 && typeof data[0] === 'object') {
            this.items = data.map(item => item.id || item.handle);
    ...
    async fetchProduct(productHandle) {
      try {
        ...
        if (this.productCache[productHandle]) {
          return this.productCache[productHandle];
        }
        
        // Use official Shopline API endpoint
        const response = await fetch(`/api/product/products.json?handle=${productHandle}`);
    ...
    add(product) {
      ...
      if (!this.isWishlisted(productHandle)) {
    ...
    remove(productHandle) {
      const index = this.items.findIndex(handle => handle === productHandle);
      if (index > -1) {
        this.items.splice(index, 1);
        this.save();
        
        // Remove from cache
        delete this.productCache[productHandle];
    ...
    showNotification(message) {
      ...
      if (this.currentNotification) {
        ...
        const existingNotification = this.currentNotification;
      ...
      // Get button settings from theme
      const buttonRadius = window.Shopline?.theme?.settings?.button_border_radius || 6;
      const buttonUppercase = window.Shopline?.theme?.settings?.button_text_uppercase || false;
      ...
      notification.style.cssText = `
        ...
        text-transform: ${buttonUppercase ? 'uppercase' : 'none'};
        letter-spacing: ${buttonUppercase ? '0.05em' : 'normal'};
        font-weight: 600;
      `;
