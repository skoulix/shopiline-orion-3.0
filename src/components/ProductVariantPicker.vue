<template>
  <div class="variant-picker">
    <!-- Single variant display - only show if variant has a meaningful title -->
    <div
      v-if="
        variants.length === 1 &&
    ...
    <div v-else-if="productOptions.length > 0" class="mt-5 space-y-4">
      <div
        v-for="option in productOptions"
        :key="option.name"
        class="variant-option"
      >
        <div class="font-medium text-sm mb-3" :style="{ color: buttonTextColor }">
          {{ option.name || "Option" }}:
          <span class="ml-1 font-medium">{{ getSelectedOptionValue(option.name) || 'None selected' }}</span>
        </div>

        <!-- Image-based variant options -->
        <div v-if="hasImageOptions(option)" class="flex flex-wrap gap-3 p-1">
          <button
        ...
        <!-- Color options (same style as other variants) -->
        <div
          ...
          <button
            ...
            :class="[
              ...
              isOptionSelected(option.name, value)
                ? 'variant-button-pill--selected'
                : '',
              !isOptionAvailable(option.name, value)
                ? 'variant-button-pill--unavailable'
                : '',
            ]"
            :style="{
              ...getVariantButtonStyle(option.name, value),
              padding: '8px 10px'
            }"
            :title="value"
        ...
        <!-- Size/Text options -->
        <div v-else class="flex flex-wrap gap-2 mt-2 p-1">
          <button
            v-for="value in option.values"
            :key="value"
            type="button"
            ...
            :style="getVariantButtonStyle(option.name, value)"
            :aria-label="`Select ${option.name} ${value}`"
            :disabled="!isOptionAvailable(option.name, value)"
            @click.stop="selectOption(option.name, value)"
          >
            {{ value }}
            <span v-if="!isOptionAvailable(option.name, value)" class="variant-strikethrough" :style="`background-color: ${buttonTextColor || '#374151'};`"></span>
          </button>
        </div>
      </div>
    </div>
    ...
    <div
      v-if="
        variants.length === 0 && productOptions.length === 0 && !isInitialized
      "
      class="variant-picker-empty"
    >
      <div class="p-4 bg-gray-50 border border-gray-200 rounded-md">
        <p class="text-sm text-gray-600">Loading variant options...</p>
      </div>
    </div>
  </div>
</template>
...
const buttonTextColor = computed(() => props.buttonTextColor || '#374151');
const buttonBorderColor = computed(() => props.buttonBorderColor || '#d1d5db');
const buttonSelectedBg = computed(() => props.buttonSelectedBg || '#111827');
const buttonSelectedText = computed(() => props.buttonSelectedText || '#ffffff');
const buttonSelectedBorder = computed(() => props.buttonSelectedBorder || '#111827');
...
const productOptions = computed(() => {
  ...
  if (variants.value.length > 0) {
    ...
    if (variants.value[0].option1) {
      const option1Values = [
        ...new Set(variants.value.map((v) => v.option1).filter(Boolean)),
      ];
      options.push({
        name: "Size",
        position: 1,
    ...
    if (variants.value[0].option2) {
      const option2Values = [
        ...new Set(variants.value.map((v) => v.option2).filter(Boolean)),
      ];
      options.push({
        name: "Color",
        position: 2,
    ...
    if (variants.value[0].option3) {
      const option3Values = [
        ...new Set(variants.value.map((v) => v.option3).filter(Boolean)),
      ];
      options.push({
        name: "Option",
        position: 3,
...
const getSelectedOptionValue = (optionName) => {
  return selectedOptions.value[optionName] || null;
};
...
const getVariantButtonStyle = (optionName, value) => {
  ...
  if (isSelected) {
    borderColor = buttonSelectedBorder.value || buttonSelectedBg.value || "rgb(17 24 39)";
    bgColor = buttonSelectedBg.value || "rgb(17 24 39)";
    textColor = buttonSelectedText.value || "rgb(255 255 255)";
  } else {
    borderColor = buttonBorderColor.value || "rgb(209 213 219)";
    bgColor = "transparent";
    textColor = buttonTextColor.value || "rgb(55 65 81)";
  }
...
const getColorSwatchStyle = (value) => {
  ...
  const isWhite =
    color.toLowerCase() === "#ffffff" || value.toLowerCase() === "white";
...
<style>
/* Only essential styles that can't be handled inline */
.variant-button-pill {
  padding: 10px 18px;
  border-radius: 6px;
  font-size: 14px;
...
.variant-button-pill:hover:not(:disabled) {
...
.variant-button-pill--unavailable:hover {
