    ...
    <div class="flex gap-3 lg:gap-4">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex justify-between gap-4 mb-3">
          <div class="flex-1">
            <h3 class="text-sm lg:text-base font-medium mb-1">
              <a
                :href="item.url"
...
const formatPrice = (price) => {
  ...
  // Handle pre-formatted prices (already includes currency symbol)
  if (typeof price === 'string' && price.includes('$')) {
    return price;
  }

  // Get current currency data from window.ShoplineCurrency (set by Handlebars)
  const currencyData = window.ShoplineCurrency || {};
  const currencySymbol = currencyData.symbol || '$';
  const currencyCode = currencyData.current || 'USD';
  
  // Cart/API prices are already in the selected currency
  const priceValue = parseFloat(price);
  
  // Get money format template
