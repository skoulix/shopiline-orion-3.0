<template>
  ...
  >
    ...
    >
      <!-- Dual image setup when hover is enabled and secondary image exists -->
      <template v-if="showSecondaryImageOnHover && secondaryImage && primaryImage">
        <!-- Primary Image -->
        <img
          :src="primaryImage"
          :alt="productData.title || 'Product image'"
          class="absolute inset-0 h-full w-full object-cover object-center transition-all duration-500 group-hover:scale-110"
      ...
      <!-- Single image when hover is disabled or no secondary image -->
      <img
      ...
      >
        <div class="relative w-5 h-5">
          ...
          <svg
            v-if="isAnimating"
            class="absolute inset-0 w-5 h-5 heart-pulse-bg"
            :class="isWishlisted ? 'text-red-500 fill-current' : 'text-gray-600 fill-current'"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
          ...
          <svg
            ...
            :class="isWishlisted ? 'text-red-500 fill-current' : 'text-gray-600'"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
      ...
      <!-- Quick View/Quick Add Button - Shows on hover -->
      <div
        v-if="productData.available && quickViewEnabled"
      ...
      >
        <span
          class="bg-white text-gray-900 px-6 py-3 font-semibold rounded-full shadow-2xl"
        >
          Out of Stock
        </span>
      </div>
    ...
    <div class="p-4 flex-1 flex flex-col">
      ...
      <div class="flex-1">
        ...
        <p
          v-if="showVendor && productData.vendor"
        ...
        <!-- Rating section with skeleton loader -->
        <div v-if="showReviews && (reviewsLoading || (reviewData && reviewData.total > 0))" class="mb-2">
          <!-- Skeleton loader while loading -->
          <div v-if="reviewsLoading" class="flex items-center gap-1 animate-pulse h-6">
            <div class="flex">
              <svg
                v-for="i in 5"
                :key="i"
            ...
            <span class="bg-gray-200 rounded h-3 w-8"></span>
          </div>
          <!-- Actual reviews when loaded -->
          <div v-else-if="reviewData && reviewData.total > 0" class="flex items-center gap-1 h-6">
            <div class="flex">
              <svg
                v-for="i in 5"
                :key="i"
            ...
            <span class="text-xs text-gray-500">({{ reviewData.total }})</span>
          </div>
        </div>
      ...
      <div class="mt-auto">
        <button
          ...
          "
          @mouseleave="
            (e) => (e.target.style.backgroundColor = 'var(--button-primary-bg)')
          "
        >
...
const showSecondaryImageOnHover = computed(() => {
  return window.Shopline?.theme?.settings?.card_show_secondary_image !== false;
});
...
const updateWishlistState = () => {
  const productHandle = productData.value.handle || productData.value.id;
  isWishlisted.value = window.OrionWishlist?.isWishlisted(productHandle) || false;
};
...
// Quick view settings
const quickViewEnabled = computed(() => {
  return window.Shopline?.theme?.settings?.enable_quick_view !== false;
});

// Quick view button text respecting uppercase setting
const quickViewButtonText = computed(() => {
  const text = 'Quick View';
  const uppercaseButtons = window.Shopline?.theme?.settings?.button_text_uppercase;
  return uppercaseButtons ? text.toUpperCase() : text;
});
...
const productData = computed(() => {
  ...
  return {
    ...
    compare_at_price:
      typeof props.compareAtPrice === "string"
...
const hasColorOptions = computed(() => {
  return productData.value.options?.some(
    (opt) =>
...
const colorOptions = computed(() => {
  const colorOption = productData.value.options?.find(
    (opt) =>
      opt.name?.toLowerCase().includes("color") ||
      opt.name?.toLowerCase().includes("colour")
  );
...
const hasMultipleVariants = computed(() => {
  // Check if product has options with multiple values
  const options = productData.value.options || [];
  if (options.length === 0) return false;
  
  // Check if any option has multiple values
...
const isOnSale = computed(() => {
  return (
    productData.value.available && compareAtPrice.value > currentPrice.value
  );
});
...
const salePercentage = computed(() => {
  ...
  return Math.round(
    ((compareAtPrice.value - currentPrice.value) / compareAtPrice.value) * 100
  );
});
...
const formatPrice = (price) => {
  ...
  if (typeof price === 'string' && (price.includes('$') || price.includes('€') || price.includes('£') || price.includes('¥') || price.includes('₹'))) {
    return price;
  }
  
  // Get current currency data from window.ShoplineCurrency
  const currencyData = window.ShoplineCurrency || {};
  let moneyFormat = currencyData.format || window.Shopline?.shop?.money_format || '${{amount}}';
  ...
  // Parse the numeric price - handle both string and number inputs
  let numPrice = price;
  ...
  // Shopline returns prices in the smallest unit (cents/pence/etc)
  // We need to divide by 100 to get the major currency unit
  ...
  // Replace the placeholder with the formatted amount
...
const handleCardMouseEnter = () => {
  ...
  if (showSecondaryImageOnHover.value && productData.value.images?.length > 1) {
    hovering.value = true;
  }
};
...
const toggleWishlist = (event) => {
  ...
  if (window.OrionWishlist) {
    const wasWishlisted = window.OrionWishlist.toggle(productData.value);
    ...
    window.dispatchEvent(new CustomEvent('wishlist:updated', { 
      detail: { productHandle: productHandle } 
    }));
  }
...
const handleQuickView = () => {
  // Open QuickView modal for products
  const handle = productData.value.handle;
  
  if (!handle) {
    console.error('No product handle found for quick view', productData.value);
    return;
  }
  
  if (window.QuickViewModal && window.QuickViewModal.openWithHandle) {
    console.log('Opening quick view for product:', handle);
    window.QuickViewModal.openWithHandle(handle);
  } else {
...
const fetchReviews = async () => {
  // Only fetch if reviews are enabled and we have a product ID
  if (!showReviews.value || !productData.value.id) return;
  ...
  try {
    const response = await fetch(
      "/api/isv-plugin/comment/front/query/summary",
      {
        ...
        body: JSON.stringify({ productId: productData.value.id }),
      }
    );
    ...
    console.debug("Could not fetch reviews for product:", productData.value.id);
  } finally {
...
onMounted(() => {
  ...
  // Listen for wishlist updates from other cards
  const handleWishlistUpdate = (e) => {
    // Update state if this product was affected
    const productHandle = productData.value.handle || productData.value.id;
    if (!e.detail || e.detail.productHandle === productHandle) {
      updateWishlistState();
    }
  };
